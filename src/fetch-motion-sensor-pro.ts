...
import { SwitchBotClient, type MotionSensorProBody } from './switchbotClient';
 ...
 */
function loadEnv(): { token: string; secret: string; deviceId: string } {
  // それぞれの環境変数を取得する。
...
async function main(): Promise<void> {
  ...
  // 実際のセンサーデータを取り出す。
  const body: MotionSensorProBody = response.body;
