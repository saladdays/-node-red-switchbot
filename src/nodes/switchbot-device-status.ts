...
import {
  SwitchBotClient,
  type MotionSensorProBody,
  type SwitchBotDeviceListBody,
} from '../switchbotClient';
import type { SwitchBotCredentialsNode } from './switchbot-credentials';

/**
 * ノードの設定用型。
 * accountはConfig Nodeへの参照ID、deviceIdは対象デバイス。
 */
export interface SwitchBotDeviceStatusConfig extends NodeDef {
  name: string;
  account: string; // Config NodeのID
  deviceId?: string;
  deviceType?: string; // 選択時に保存されるデバイスタイプ
  deviceName?: string; // 選択時に保存されるデバイス名（UI表示用）
}

interface DeviceListResponse {
  ok: boolean;
  devices?: { deviceId: string; deviceName: string; deviceType: string }[];
  message?: string;
}
 ...
 */
function mapLightLevelToLabel(level: number | undefined): 'bright' | 'dim' | 'dark' | 'unknown' {
  if (level === undefined || level === null) return 'unknown';
  if (level >= 2) return 'bright';
  if (level === 1) return 'dim';
 ...
 */
function normalizePayload(body: MotionSensorProBody) {
 ...
 */
export function registerSwitchBotDeviceStatusNode(RED: NodeAPI): void {
  // デバイス一覧取得エンドポイント
  RED.httpAdmin.post('/switchbot-device-status/devices', async (req, res) => {
    try {
      ...
      const body: SwitchBotDeviceListBody = response.body;
      ...
      res.json({ ok: true, devices } satisfies DeviceListResponse);
    } catch (error) {
  ...
  function SwitchBotDeviceStatusNode(this: Node, config: SwitchBotDeviceStatusConfig): void {
    ...
    node.on(
      ...
      async (
        ...
        send?: (msg: NodeMessageInFlow) => void,
        done?: (err?: Error) => void,
      ): Promise<void> => {
        ...
        try {
          ...
          // フォールバック: 環境変数
          if (!token) token = process.env.SWITCHBOT_TOKEN || '';
          ...
          if (response.statusCode !== 100) {
            ...
            node.error(
              `SwitchBot API error: statusCode=${response.statusCode}, message=${response.message}`,
              msg,
            );
            finisher();
            return;
